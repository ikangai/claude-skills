...
| zai-org/glm-4.7-flash | flash-class, 202K ctx | Chat-template leak workaround **confirmed working** (2026-06-10 eval): max_tokens ≥4K + truncate output at the first `<\|` template token → clean output, best faithfulness of the author's local pool (summarization 6.5). Remaining limit is reasoning latency: ~80–160s per call, can hit a 300s timeout on a 25K-token prompt | Route small/medium narrative-synthesis tasks only, with the two-part workaround applied (the diary skill's `synthesize-local.sh` in this repo is a reference implementation). Latency-sensitive work goes elsewhere. |
